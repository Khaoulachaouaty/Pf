...
const AddEq = () => {
  ...
  const handleSaveDialog = (e) => {
    ...
    if (eqImage) {
      ...
      const blob = new Blob(
        [new Uint8Array(atob(eqImage).split('').map(char => char.charCodeAt(0)))],
        { type: 'image/jpeg' } // Modifier le type d'image si nécessaire
      );
    }

    console.log(eqImage,"immmm")
  ...
  const [eqImage, setEqImage] = React.useState(null);
  ...
  return (
    <Container>
      ...
      <Grid item xs={2}>
        ...
        {eqImage && (
          <IconButton onClick={handleImageCancel} aria-label="delete" size="small">
            <DeleteIcon />
          </IconButton>
        )}
      </Grid>
